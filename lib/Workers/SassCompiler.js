...
const { error, info, success, warning } = require('../Helpers/Logger');
...
class SassCompiler extends Worker {
  ...
  async start() {
    ...
    const entries = this.services.Filesystem.publish(this.name);
      ...
      render(
        {
          ...
          sourceMap: this.services.Store.use('config', 'devMode'),
        },
        async (exception, chunk) => {
          /**
           * Catch the exceptions from the current entry render in order to
          ...
          if (exception) {
            ...
            this.services.Filesystem.writeBatch(
  ...
  outputExceptions() {
    ...
    if (this.exceptions && this.exceptions.sass.length) {
      ...
      if (this.services.Store.use('config', 'devMode')) {
