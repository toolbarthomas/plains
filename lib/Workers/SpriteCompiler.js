  ...
  buildSprite(chunk, destinationDirectory) {
    return new Promise(callback => {
      ...
      const sprite = chunk.reduce(
        (instance, item) => {
          return instance.add(
            `${this.config.options.prefix}${basename(item.sourcePath)}`,
            extname(item.sourcePath),
            item.data
          );
        },
        svgstore({
      ...
      if (sprite) {
        const filename = basename(this.config.options.name, extname(this.config.options.name));
