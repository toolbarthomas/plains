...
class Worker {
  constructor(services, ...options) {
    if (!(services instanceof Object)) {
      error(`No services have been defined for ${this.constructor.name}`);
    }
  ...
  beforeMount() {
    ...
    if (!this.services.FileSystem.hasStack(this.name)) {
    ...
    log('Worker ready to be mounted', this.name);

    if (!this.options || !this.options.disablePrePublishCallback) {
   ...
   */
  resolve(method) {
    this.services.Contractor.resolve(this.name, method);
  }
   ...
   */
  reject(method) {
    this.services.Contractor.reject(this.name, method);
  }
  ...
  defineWorkerEnvironment(environment) {
    ...
    return warning(`${this.name} has no defined environment mode.`);
  }
}
