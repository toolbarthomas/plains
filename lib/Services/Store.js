...
class Store {
  ...
  commit(state, key) {
    if (!(state instanceof Object)) {
      throw new Error('Unable to commit to Store, the initial state is not a valid Object');
    }
    ...
    if (key) {
      if (this.instances[key]) {
        log('Store updated', key);
      } else {
        log('Store created', key);
      }
  ...
  use(key) {
    if (key && !this.instances[key]) {
      return error(`Unable to use Store entry: '${key}', the commit does not exists.`);
    }
