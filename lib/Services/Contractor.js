...
class Contractor {
  ...
  subscribe(name, hook, handler, ...args) {
    if (!name) {
      return error(
        'A name is required while subscribing a new handler to Contractor.'
      );
    }
    ...
    this.instances[name].hook = Array.isArray(hook)
      ? hook
      : hook
      ? [hook]
      : ['default'];
  ...
  /**
   * Returns the defined hook for the named subscription.
   *
   * @param {String} name The subscription to return the hook from
   */
  hook(name) {
    return this.instances[name] && this.instances[name].hook
      ? this.instances[name].hook
      : false;
  }
  ...
  resolve(name, ...args) {
    if (
      this.instances[name] &&
      typeof this.instances[name].resolve === 'function'
    ) {
  ...
  reject(name, ...args) {
    if (
      this.instances[name] &&
      typeof this.instances[name].reject === 'function'
    ) {
