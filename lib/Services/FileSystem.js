...
class FileSystem {
  ...
  insertEntry(stack, entries) {
    ...
    const entryCollection = entryArray.map(entry => {
      ...
      const src =
        entry.indexOf('*') >= 0
          ? sync(resolve(this.src, initialEntry.replace('**.', '**/*.')))
              .map(glob => resolve(glob))
  ...
  async copy(entries) {
    if (!entries) {
      error(`Cannot duplicate entry since it does not exists.`);
    }

    const queue = (Array.isArray(entries) ? entries : [entries]).map(
      entry =>
        new Promise(callback => {
          const path = this.resolveEntryPath(entry);

          if (existsSync(path)) {
            warning(`${entry.path} has already been duplicated.`);
            return callback();
          }

          log('Duplicating file', entry.path);

          return mkdirp(dirname(path), async dirError => {
            if (dirError) {
              error(dirError);
            }

            await copyFile(entry.path, path, fileError => {
              if (fileError) {
                error(fileError);
              }

              log('File duplicated', path);
  ...
  resolveEntryPath(entry, name) {
    ...
    // Defines the optional name for the current entry, use the default
