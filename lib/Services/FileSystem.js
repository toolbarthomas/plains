...
class FileSystem {
  ...
  resolveDestinationDirectory(entries) {
    if (!entries || !entries.length) {
      error(['Unable to defined destination path.', ' entries to resolve have been defined.']);
    }

    const queue = (Array.isArray(entries) ? entries : [entries]).map(entry =>
      dirname(this.resolveEntryDestination(entry))
    );
    ...
    const destinationDirectory = resolve(sep, ...matches);

    /**
     * Throw an Exception if the resolved directory is not within the same
     * path as the defined build directory.
     */
    if (!destinationDirectory || destinationDirectory.indexOf(this.dist) < 0) {
