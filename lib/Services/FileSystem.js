const { copyFile, existsSync, writeFile, readFile, statSync } = require('fs');
const { sync } = require('glob');
const { basename, dirname, extname, join, normalize, relative, resolve, sep } = require('path');
...
class FileSystem {
  ...
  /**
   * Defines the entry directory from the defined entry paths.

   * @param {*} entries The entries to resolve the directory from.
   */
  resolveEntryDirectory(entries) {
    if (!entries || !entries.length) {
      return null;
    }

    const queue = (Array.isArray(entries) ? entries : [entries]).map(entry => dirname(entry));
    ...
    // Resolve the actual directory back to an absolute FileSystem path.
    const resolvedDirectory = resolve(sep, ...matches);

    /**
     * Throw an Exception if the resolved directory is not within the same
     * path as the defined build directory.
     */
    if (!resolvedDirectory || resolvedDirectory.indexOf(this.src) < 0) {
      error('The destination directory does not resolve the the build directory');
    }
  ...
  /**
   * Define a destination path that is based of the first matching parent
   * directory for each defined entry.
   *
   * @param {Array} entries Array with FileSystem entries where the resolved
   * directory will be defined from.
   */
  resolveDestinationDirectory(entries) {
    const directory = this.resolveEntryDestination(this.resolveEntryDirectory(entries));

    /**
     * Throw an Exception if the resolved directory is not within the same
     * path as the defined build directory.
     */
    if (!directory || directory.indexOf(this.dist) < 0) {
      error('The destination directory does not resolve the the build directory');
    }
