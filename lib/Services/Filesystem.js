const { copyFile, existsSync, writeFile } = require('fs');
...
class Filesytem {
  ...
  resolveSource(relativePath) {
    ...
    return src
      ? src
      : error(`No source path has been defined to the Filesystem.`);
  }
  ...
  resolveDestination() {
    return this.dist
      ? this.dist
      : error(
          'There is no destination path defined to the Filesystem service.'
        );
  }
  ...
  defineEntry(name, entry) {
      ...
      queue.map(path => {
        ...
        const initialPath =
          relativePath.indexOf(this.resolveSource(true)) === 0
            ? relativePath
                .replace(this.resolveSource(true), '')
                .replace(sep, '')
            : path;

        return path.indexOf('*') >= 0
          ? sync(resolve(this.resolveSource(), initialPath)).map(glob =>
              resolve(glob)
            )
    ...
    const commit = subscription.filter(initialEntry => {
      const duplicates = initialSubscription.filter(
        e => e.path === initialEntry.path
      );
    ...
    if (commit.length) {
      log(
        'Filesystem updated',
        `Defined ${commit.length} new ${
          commit.length === 1 ? 'entry' : 'entries'
        } for ${name}`
      );
  ...
  resolveEntryDestination(path) {
    return join(
      ...
      dirname(relative(this.resolveSource(), path))
    );
  }

  /**
   * Returns the base directory from the given path array.
   *
   * @param {Array} paths The paths to define the base directory from.
   */
  resolveInstance(paths) {
    if (!paths || !paths.length) {
      return null;
    }

    const queue = (Array.isArray(paths) ? paths : [paths]).map(path =>
      dirname(path)
    );
    ...
    /**
     * Place each matching directory within the matches array in order
     * to resolve the path to the destination directory for the current entries.
     */
    queue.forEach((entry, index) => {
      const directories = entry.split(sep).filter(i => i);

      if (index === 0) {
        matches.push(...directories.filter(dir => dir));
      } else {
        directories.forEach((dir, i) => {
          if (matches.indexOf(dir) !== i) {
            matches.splice(i);
          }
        });
      }
    ...
    // Resolve the actual directory back to an absolute path.
    const resolvedDirectory = resolve(sep, ...matches);

    /**
     * Return the resolved source directory or throw an Exception if the
     * resolved directory is not within the same path as the defined build
     * directory.
     */
    return resolvedDirectory && resolvedDirectory.indexOf(this.src) >= 0
      ? resolvedDirectory
      : error(
          'The destination directory does not resolve the the build directory'
        );
  }

  /**
   * Returns an array with all the subscribed instances that have actual paths
   * defined.
   *
   * @returns {Array} Contains all the subscribed instances with paths.
   */
  list() {
   ...
   * @param {String} name The optional filename to include within the resolve.
   */
  ...
  /**
   * Resolve the given paths relative to the defined destination directory &
   * synchronize all the defined entry files to it.
   */
  copy(path) {
    return new Promise((callback, reject) => {
      if (!path) {
        error('No path has been defined to duplicate');
      }

      if (!existsSync(path)) {
        error('Unable to copy file, it does not exists.');
      }
      ...
      if (existsSync(destination)) {
        ...
        mkdirp(dirname(destination), async err => {
          ...
          await copyFile(path, destination, err2 => {
            if (err2) {
              error(err2);
            }

            log('File copied', destination);
