...
class Plains {
  ...
  async start() {
    ...
    await this.mount('workers');

    // Mount each Plugin instance.
  ...
  async mount(mountpoint) {
    if (!(this[mountpoint] instanceof Object)) {
      return;
    }

    const queue = [];

    Object.keys(this[mountpoint]).forEach(name => {
      const instance = this[mountpoint][name];
